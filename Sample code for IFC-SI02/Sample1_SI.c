...
//=====================================================================================
#define add_si1		0b000001	// Please make sure the communication address on 
...
{//--------------------------------------------------------------------------------
	...
	delay(10000);				// waiting for slave card to ready
	...
	while(1)
	{
		...
		if(sw1==0)							// Test whether SW1 on IFC-MB00 is pressed
		{
			...
			i=0;
				...
				led4=si_1_runstat(add_si1);			// Turn ON led4 when motor SI1 run	
				led5=si_2_runstat(add_si1);			// Turn ON led5 when motor SI2 run
				...
				led4=si_1_runstat(add_si1);			// turn OFF led4 when motor SI1 stop
				led5=si_2_runstat(add_si1);			// turn OFF led5 when motor SI2 stop
				...
				delay(50000);					// delay, wait time
				led4=si_1_runstat(add_si1);			// turn OFF led4 when motor SI1 brake
				led5=si_2_runstat(add_si1);			// turn OFF led5 when motor SI2 brake
		...
		if(sw2==0)							// Test whether SW2 on IFC-MB00 is pressed
		{
			...
			buzzer = 0;						// OFF buzzer
				...
				while(si_1_enval(add_si1)<50000);		// wait for enconder tasks are done
		...
		if(sw3==0)			   			// Test whether SW3 on IFC-MB00 is pressed
		{
			...
			si_1_setacc(add_si1,250);
			si_2_setacc(add_si1,250);
			si_1_cw(add_si1);
			si_2_cw(add_si1);
			...
			{		
				...
				delay(50000); 			//delay for speed increase time
			}
